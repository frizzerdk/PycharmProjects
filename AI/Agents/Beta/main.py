...
for M_CONFIG in configs:
    ...
    run = wandb.init(project="AC_RL_BipedalWalker-v3_beta",config=M_CONFIG, reinit=True)
    ...
    for episode in range(start, M_CONFIG['max_episodes']):
        ...
        for step in range(M_CONFIG['max_steps']):
            ...
            # Reset model every 10 episodes
            if episode % 10 == 100:
                trainer.reset_models()
            # Check if episode is done
            weak_perfomance=sum_reward<30 and step>300
            if done or weak_perfomance:
                break

        # Optimize the trainer
        if (episode+1) % M_CONFIG['exploit_freq'] == 0:
            best_val_loss,final_epoch, critic_val_loss ,actor_val_loss=trainer.optimize()
            mlog.log({
                "best_val_loss": best_val_loss,
                "final_epoch": final_epoch,
                "critic_val_loss": critic_val_loss,
                "actor_val_loss": actor_val_loss,
            })
